...
export function authenticate(req: Request, res: Response, next: NextFunction) {
  ...
  if (!header?.startsWith("Bearer ")) return res.status(401).json({ error: "Falta el token" });
  try {
...
export function authorize(...roles: Rol[]) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!roles.includes(req.usuario.rol)) return res.status(403).json({ error: "No tenés permiso para esta operación" });
    next();
  };
}
